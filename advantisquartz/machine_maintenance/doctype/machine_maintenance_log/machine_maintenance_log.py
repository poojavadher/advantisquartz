...
from advantisquartz.machine_maintenance.doctype.machine_maintenance_schedule.machine_maintenance_schedule import calculate_next_due_date

class MachineMaintenanceLog(Document):
	def validate(self):
	...
	def update_maintenance_task(self):
		...
		if self.maintenance_status == "Cancelled":
			...
			asset_maintenance_doc.save()
		asset_maintenance_doc = frappe.get_doc("Machine Maintenance Schedule", self.machine_maintenance_schedule)
		asset_maintenance_doc.save()
...
def get_maintenance_tasks(doctype, txt, searchfield, start, page_len, filters):
	asset_maintenance_tasks = frappe.db.get_values(
		"Asset Maintenance Task", {"parent": filters.get("machine_maintenance_schedule")}, "maintenance_task"
	)
