...
class MachineMaintenanceSchedule(Document):
	def validate(self):
	...
	def sync_maintenance_tasks(self):
		...
		for task in self.get("asset_maintenance_tasks"):
			...
			update_maintenance_log(
				machine_maintenance_schedule=self.name, item_code=self.item_code, item_name=self.item_name, task=task
			)
		asset_maintenance_logs = frappe.get_all(
			"Machine Maintenance Log",
			fields=["name"],
			filters={"machine_maintenance_schedule": self.name, "task": ("not in", tasks_names)},
		)
		if asset_maintenance_logs:
			for asset_maintenance_log in asset_maintenance_logs:
				maintenance_log = frappe.get_doc("Machine Maintenance Log", asset_maintenance_log.name)
...
def assign_tasks(asset_maintenance_name, assign_to_member, maintenance_task, next_due_date):
	...
	args = {
		"doctype": "Machine Maintenance Schedule",
...
def update_maintenance_log(machine_maintenance_schedule, item_code, item_name, task):
	...
	if not asset_maintenance_log:
		...
		maintenance_log = frappe.get_doc("Machine Maintenance Log", asset_maintenance_log)
...
@frappe.whitelist()
def get_maintenance_log(item_code):
	return frappe.db.sql(
		"""
        select maintenance_status, count(item_code) as count,item_code
        from `tabMachine Maintenance Log`
        where item_code=%s group by maintenance_status""",
		(item_code),
		as_dict=1,
	)