...
frappe.ui.form.on('Machine Maintenance Schedule', {
	setup: (frm) => {
		frm.set_query("assign_to", "asset_maintenance_tasks", function(doc) {
			return {
				query: "advantisquartz.machine_maintenance.doctype.machine_maintenance_schedule.machine_maintenance_schedule.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if(!frm.is_new()) {
			frappe.call({
				method: 'advantisquartz.machine_maintenance.doctype.machine_maintenance_schedule.machine_maintenance_schedule.get_maintenance_log',
					...
					(r.message || []).forEach(function(d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="frappe.set_route('List', 'Machine Maintenance Log',
									{'item_code': '${d.item_code}','maintenance_status': '${d.maintenance_status}' });">
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return frappe.call({
			method: 'advantisquartz.machine_maintenance.doctype.machine_maintenance_schedule.machine_maintenance_schedule.calculate_next_due_date',
			args: {
