frappe.views.ImageView = class ImageView extends frappe.views.ListView {
    ...
    set_fields() {
        if (this.doctype === 'Employee') {
            this.fields = [
                ...
                "personal_email", // Add personal_email field
                "cell_number", // Add cell_number field
            ];
        } else {
            this.fields = [
                "name",
    ...
    render_image_view() {
        ...
        `);
            ...
            // Set height for the image-view-item
            this.$result.find('.image-view-item').css('height', '330px'); // Adjust the height as per your requirement
    ...
    item_details_html(item) {
        ...
        // let info_html = super.item_details_html(item);
        let set = false;
        ...
        if (this.doctype === 'Employee') {
            if (item.designation) {
                info_html += `<li>${__("Designation")}: ${item.designation}</li>`;
            }
            if (item.department) {
                info_html += `<li>${__("Department")}: ${item.department}</li>`;
            }
            if (item.branch) {
                info_html += `<li>${__("Branch")}: ${item.branch}</li>`;
            }
            if (item.personal_email) {
                info_html += `<li>${__("Email")}: ${item.personal_email}</li>`;
            }
            if (item.cell_number) {
                info_html += `<li>${__("Mobile")}: ${item.cell_number}</li>`;
            }
            info_html += `</ul></div>`;
            return info_html;
        } else {
    ...
    item_html(item) {
        ...
        if (this.doctype === 'Employee') {
            ...
            const _html = item._image_url
                ? `<img data-name="${encoded_name}" src="${item._image_url}" alt="${title}" style="height: 170px; width: 200px;">`
